...
// 상태관리에서 변경 부분 반영을 위한 build 메서드를 분리하여
// 반영하는 서브 클래스 
class _CartDetailState extends State<CartDetail> {
  // 선택된 버튼 순번을 저장하는 변수
  int sequenceNum = 0;
  // -> 이 클래스 내부에서 선언하여 사용하는 변수의
  // 값이 변경되면 이 변수를 사용하는 위젯이 자동 업데이트 된다!
  ...
  Map<String, List> goodsColor = {
    ...
    "Honda motorcycle": [Colors.grey.shade400, Colors.blue, Colors.purple],
    "Tesla Model3": [Colors.pink, Colors.yellow, Colors.green],
    "Cessna 150": [Colors.orange, Colors.blueAccent, Colors.blueGrey, Colors.lightGreen],
  };
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        // 1. 상품 이미지 : _buildPicture()
        _buildPicture(),
        // 2. 선택 버튼 : _buildSelector() -> _buildSelectButton()
        _buildSelector(),
        // 3. 상품 정보 : _buildCartInfo()
  ...
  Widget _buildPicture(){
    return Padding(
      ...
      AspectRatio(
        ...
        child: Image.asset(
          // 위에 셋팅된 이미지를 호출
          // List 형 변수의 순번으로 상태변경 클래스 내부 변수를
  ...
  Widget _buildSelectButton(int seq, IconData mIcon){
    // seq 변수 : 버튼 클릭시 변경할 순번 변수값 셋팅
    // mIcon 변수 : 버튼 아이콘
    return Container(
      width: 70,
      height: 70,
      decoration: BoxDecoration(
        // 버튼 배경색상은 선택된것과 일반적인 것으로 구분
        // 현재 버튼하고 선택버튼 순번과 같으면 엑센트 색 넣기
      ...
      child: IconButton(
        ...
        onPressed: (){
          // 여기가 매우매우매우 중요함
          // 버튼 클릭시 sequenceNum 변수값을 업데이트 한다.
  ...
  // 3. 상품 정보 위젯 생성 메서드 : _buildCartInfo()
  // 상품명 + 상품가격 + 별점 + 리뷰수 + 색상옵션 + 버튼
  ...
  Widget _buildNamePrice(){
    return Padding(
      ...
      child: Row(
        // 메인 축 정렬 양쪽끝 (사이간격만 주기)
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          // 1. 상품명 : selectedTit 리스트형 변수값 읽어옴
          // sequenceNum의 순번값이 변경될때 이것도 업데이트 됨!!!
          Text(
  ...
  Widget _buildOption(){
    // 선택색상 정보 변수에 셋업하기 (해당 타이틀명의 GoodsColor 맵)
  ...
  Widget _buildDetailIcon(Color mColor) {
    return Padding(
      ...
      // 스택은 아래로부터 위로 겹쳐져 쌓이는 형태를 이름
      // 내부에 겹쳐질 위젯은 Positioned() 위젯을 사용함
      // 이 위젯은 웹에서 absolute 포지션과 유사함!
      // (top, left 사용가능)
      child: Stack(
        children: [
          Container(
